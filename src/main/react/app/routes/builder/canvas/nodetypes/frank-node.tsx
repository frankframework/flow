        ...
        {properties.data.attributes &&
          Object.entries(properties.data.attributes).map(([key, value]) => (
            <div key={key} className="my-1 w-full max-w-full px-1">
              <p className="overflow-hidden text-sm overflow-ellipsis whitespace-nowrap text-gray-1000">{key}</p>
              <p className="overflow-hidden text-sm overflow-ellipsis whitespace-nowrap">{value}</p>
            </div>
          ))}
        {properties.data.children.length > 0 && (
          <div className="w-full p-4">
            <div className="w-full rounded-md border-border bg-white p-4 shadow-[inset_0px_2px_4px_rgba(0,0,0,0.1)]">
              {properties.data.children.map((child) => (
                <div
                  key={child.type}
                  className="mb-1 max-w-max rounded-md border-1 border-border bg-white"
                  ...
                  {child.attributes &&
                    Object.entries(child.attributes).map(([key, value]) => (
                      <div key={key} className="my-1 px-1">
                        <p className="overflow-hidden text-sm overflow-ellipsis whitespace-nowrap text-gray-1000">
                          {key}
                        </p>
                        <p className="overflow-hidden text-sm overflow-ellipsis whitespace-nowrap">{value}</p>
                      </div>
      ...
      {isHandleMenuOpen && (
        ...
        >
          <ul>
            ...
            <li
              className="cursor-pointer rounded-t-md p-2 hover:bg-border"
              onClick={() => handleMenuClick('success')}
            >
              Success
            </li>
            <li className="cursor-pointer p-2 hover:bg-border" onClick={() => handleMenuClick('failure')}>
              Failure
            </li>
            <li className="cursor-pointer p-2 hover:bg-border" onClick={() => handleMenuClick('exception')}>
              Exception
            </li>
            <li className="cursor-pointer rounded-b-md p-2 hover:bg-border" onClick={() => handleMenuClick('custom')}>
              Custom
            </li>
          </ul>
        </div>
