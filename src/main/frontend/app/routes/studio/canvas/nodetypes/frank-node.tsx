...
export default function FrankNode(properties: NodeProps<FrankNodeType>) {
  ...
  return (
    <>
      ...
      <div
        className={`bg-background border-border relative flex w-full flex-col items-center overflow-x-visible rounded-md border shadow-[0_4px_6px_1px_rgba(0,0,0,0.05)] ${isManuallyResized ? 'h-full overflow-y-hidden' : 'overflow-y-visible'}`}
        style={{
      ...
      >
        ...
        {(properties.data.children.length > 0 || dragOver || canDropDraggedElement) && (
          <div className="w-full p-4">
            <div className="border-border/40 bg-background w-full rounded-md border p-4 shadow-[inset_0_2px_4px_rgba(0,0,0,0.15)]">
