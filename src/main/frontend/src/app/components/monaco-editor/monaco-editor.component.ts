...
export class MonacoEditorComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  _editorSubject = new ReplaySubject<monaco.editor.IStandaloneCodeEditor>(1);
  editor$ = this._editorSubject.asObservable();
  ...
  private initializeEditor(): void {
    ...
    this._editorSubject.next(this.editor);
  }
  ...
  private setLineNumberInRoute(startLineNumber: number, endLineNumber: number | undefined = undefined): void {
  ...
  private async setValue(content: string): Promise<void> {
  ...
  private findMatchForRegex(regexp: string): monaco.editor.FindMatch[] | undefined {
